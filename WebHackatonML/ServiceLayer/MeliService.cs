...
namespace MeliSample
{
	public class MeliService
	{
        ...
        //https://api.mercadolibre.com/orders/search?seller=160252486&access_token=$ACCESS_TOKEN
        public List<Order> getOrders(string clientID)
        {
            string resource = "orders/search"; 
            List<Parameter> parametros = new List<Parameter>(); 
            parametros.Add(new Parameter(){Name = "seller", Type = ParameterType.GetOrPost , Value = clientID});
            parametros.Add(new Parameter() { Name = "access_token", Type = ParameterType.GetOrPost, Value = meliService.service.AccessToken });

            IRestResponse result = service.Get(resource, parametros);

            JObject data = JObject.Parse(result.Content);                                
            
            List<Order> listaOrdenes = JsonConvert.DeserializeObject<List<Order>>(JObject.Parse(result.Content)["results"].ToString());
